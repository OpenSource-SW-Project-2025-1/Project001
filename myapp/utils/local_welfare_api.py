...
# API URL 및 서비스 키 정의
base_list_url = "http://apis.data.go.kr/B554287/LocalGovernmentWelfareInformations/LcgvWelfarelist"
base_detail_url = "http://apis.data.go.kr/B554287/LocalGovernmentWelfareInformations/LcgvWelfaredetailed"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) >= 7 and sys.argv[6]:
        params["ctpvNm"] = sys.argv[6]  # 여섯 번째 인자: 시도명
    if len(sys.argv) >= 8 and sys.argv[7]:
        params["sggNm"] = sys.argv[7]  # 일곱 번째 인자: 시군구명

    # api 요청 실행
    response = requests.get(base_list_url, params=params)

    # 결과 저장 경로: media/local_api_result.txt
